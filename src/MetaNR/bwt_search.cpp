...
void BWT_Search(int64_t myPos, char *seq)
{
	uint8_t nt;
	vector<int> TaxVec;
	bwtintv_t ik, ok[4];
	...
	if (pos == ReadLength)
	{
		MinPos = DoubleRefSeqSize; freq = (int)ik.x[2]; TaxVec.resize(freq);

		for (i = 0; i < freq; i++)
		{
			if ((gPos = bwt_sa(ik.x[0] + i)) >= RefSeqSize) gPos = DoubleRefSeqSize - gPos - ReadLength;
			// ShowReadSequence(gPos);
			RefTaxArr[gPos] = 0; if (gPos < MinPos) MinPos = gPos;
			TaxVec[i] = (iter = RefPosTaxMap.lower_bound(gPos))->second;
			//if((iter = RefPosTaxMap.lower_bound(gPos))->second != 0) TaxVec.push_back(iter->second);
		}
		sort(TaxVec.begin(), TaxVec.end());
		//for (vector<int>::iterator iter = TaxVec.begin(); iter != TaxVec.end(); iter++) printf("%d ", *iter); printf("\n");
		TaxVec.erase(unique(TaxVec.begin(), TaxVec.end()), TaxVec.end());
		//for (vector<int>::iterator iter = TaxVec.begin(); iter != TaxVec.end(); iter++) printf("%d ", *iter); printf("\n");
		RefTaxArr[MinPos] = Find_LCA(TaxVec);
	}
	else RefTaxArr[myPos] = 0;
}
