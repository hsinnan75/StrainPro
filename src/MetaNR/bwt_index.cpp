...
bwaidx_t *bwa_idx_load(const char *hint)
{
	...
	fprintf(stderr, "Step2. Load the reference index.\n"); 
...
void *IdvLoadReferenceSequences(void *arg)
{
	...
	my_id = (int*)arg;
	for (fPos = *my_id; fPos < RefSeqSize; fPos += iThreadNum)
	{
		base = RefIdx->pac[fPos >> 2] >> ((~fPos & 3) << 1) & 3;
		switch (base)
		{
		case 0: RefSequence[fPos] = 'A'; break;
		case 1: RefSequence[fPos] = 'C'; break;
		case 2: RefSequence[fPos] = 'G'; break;
		case 3: RefSequence[fPos] = 'T'; break;
